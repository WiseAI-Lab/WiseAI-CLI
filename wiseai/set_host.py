...
from wiseai.utils.config import WISEAI_DIR, HOST_URL_FILE_PATH


@click.group(invoke_without_command=True)
@click.option(
    "-sh", "--set-host", help="Set the Host URL of the WiseAI Instance."
)
def host(set_host):
    """
        ...
        if not os.path.exists(HOST_URL_FILE_PATH):
            echo(
                style(
                    ...
                    "The CLI would be using https://localhost:8000 as the default url.",
                    bold=True,
                )
            )
        else:
            with open(HOST_URL_FILE_PATH, "r") as fr:
                try:
                    ...
                    echo(
                        style(
                            "{} is the Host URL of WiseAI.".format(data),
                            bold=True,
                        )
                    )
                except (OSError, IOError) as e:
